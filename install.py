...
import os
rPackages = ["libcurl4", "libxslt1-dev", "libgeoip-dev", "libonig-dev", "e2fsprogs", "wget", "mcrypt", "nscd", "htop", "zip", "unzip", "mc", "mariadb-server", "libpng16-16", "python3-paramiko", "python-is-python3", "libzip5"]
...
def prepare(rType="MAIN"):
    ...
    if not is_installed("libzip5"):
        ...
        try:
            # 首先尝试从系统仓库安装
            subprocess.run("apt-get install -y libzip5 > /dev/null 2>&1", shell=True, check=True)
        except subprocess.CalledProcessError:
            # 如果失败，尝试下载特定版本
